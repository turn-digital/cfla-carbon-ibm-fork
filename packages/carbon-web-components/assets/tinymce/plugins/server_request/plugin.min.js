tinymce.PluginManager.add('server_request', function (editor) {
  ...
  var invalidateQueriesFunc = window['config_invalidateQueriesFunc'] || null;
    ...
    fetch(requestObject.urlToRequest, {
      ...
      .then((result) => {
        ...
        if (invalidateQueriesFunc) {
          console.log('change status setstate result.server', result.status);
          invalidateQueriesFunc(result.status); // Call the function of setState to change status
        }
      })
      .catch((error) => {
        editor.mode.set('design');
