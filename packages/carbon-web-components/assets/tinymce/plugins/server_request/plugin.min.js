tinymce.PluginManager.add('server_request', function (editor) {
  ...
  var onServerErrorTextNotification =
    window['config_' + editor.id].onServerErrorTextNotification || null;
  var invalidateQueriesFunc =
    window['config_' + editor.id + '_invalidateQueriesFunc'] || null;

  console.log('fucn console.log on init', invalidateQueriesFunc);
  console.log('fucn console.log on init typeof', typeof invalidateQueriesFunc);
  console.log('window in plugin', window);
  ...
  editor.on('blur', function () {
    if (invalidateQueriesFunc) {
      console.log(
        'console.log on blur that function is here',
        invalidateQueriesFunc
      );
      console.log(editor.id);
      invalidateQueriesFunc(Math.floor(Math.random() * 1000000) + 1); // Call the function
    }
