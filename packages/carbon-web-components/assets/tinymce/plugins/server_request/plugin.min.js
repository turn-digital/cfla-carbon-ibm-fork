tinymce.PluginManager.add('server_request', function (editor) {
  ...
  function insertServerInfoHtml() {
    ...
    var serverInfoHtml = `
        <div style="display: flex; justify-content: space-between;">
          <p style="color: #5F6367; line-height: 16px; margin-top: -20px; font-size: 12px; font-weight: 400; margin: 0;" id="server-last-editor-${editorId}">
      ...
      onServerLastEditor?.date || ''
            ...
            ${
              requestObject
                ? `<div style="display: flex;"><p id="server-saved-text-${editorId}" style="display: none; color: #5F6367; font-size: 12px; font-weight: 400; margin-right: 10px;"></p><cds-button class="test-class-1" id="save-button-${editorId}" disabled size="sm" style="margin-top: 2px;">Saglabāt</cds-button></div>`
                : ''
            }
        </div>
    `;
  ...
  function makePostRequest(requestObject, data, propsBodyObj) {
    ...
    fetch(requestObject.urlToRequest, {
      ...
      .then((result) => {
        ...
        // to disable the button and change text to "Saglabāts"
        var saveButton = editor
          .getElement()
          .parentNode.querySelector(`#save-button-${editorId}`);
        var serverSavedText = editor
          .getElement()
          .parentNode.querySelector(`#server-saved-text-${editorId}`);

        saveButton.setAttribute('disabled', 'true');
        serverSavedText.textContent = 'Saglabāts';
        serverSavedText.style.display = 'block';
      ...
      .catch((error) => {
        editor.mode.set('design');
        editor.notificationManager.open({
          text: 'Kļūda savienojumā ar serveri',
          type: 'error',
        });
      });
  }
  ...
  editor.on('init', function () {
    ...
    // function to disable and enable save button
    const initialContent = editor.getContent({ format: 'html' }).trim();
    ...
    editor.on('keyup', function () {
      ...
      if (saveButton) {
        // Enable save button if there's any change in content
        if (currentContent !== initialContent) {
          saveButton.removeAttribute('disabled');
          serverSavedText.textContent = 'Nav saglabāts';
          serverSavedText.style.display = 'block';
        } else {
          saveButton.setAttribute('disabled', 'true'); // Optionally re-disable
          serverSavedText.style.display = 'none';
        }
      }
    });
  });
